...
class JobFilterEngine:
    # 1. Palavras-chave estritas de IA / Machine Learning / Data Science / GenAI / NLP
    AI_KEYWORDS = [
        ...
        r"\bdata science\b", r"\bdata scientist\b", r"\bdata engineer(ing)?\b", r"\bdata analyst\b",
        r"\bciência de dados\b", r"\bcientista de dados\b", r"\bengenheiro de dados\b",
        r"\btransformers?\b", r"\bprompt engineer(ing)?\b", r"\bmlops\b", r"\bai engineer(ing)?\b",
        r"\bresearch scientist\b", r"\binvestigador\b", r"\btech & ai\b"
    ]

    # 2. Palavras-chave de Engenharia de Software / Sistemas / Backend / Cloud (para empresas com rating >= 3.1)
    ...
    # 3. OBRIGATÓRIO NO TÍTULO: O Título TEM de ter um destes termos explícitos de perfil inicial
    STRICT_TITLE_KEYWORDS = [
        r"\bjunior\b", r"\bjúnior\b", r"\bjr\.?\b", r"\btrainee\b", r"\bintern(ship)?\b",
        r"\bestágio\b", r"\bestagio\b", r"\bestagiário\b", r"\bestagiario\b",
        r"\bgraduate\b", r"\bentry[- ]level\b", r"\bearly[- ]career\b", r"\bstudent\b",
        r"\bbolseir[oa]\b", r"\bbolsa\b", r"\biefp\b", r"\bcurricular\b", r"\bprofissional\b",
        r"\bresearch scientist\b"
    ]

    # 4. EXCLUSÕES: Nunca permitir estes termos no título
    EXCLUDE_KEYWORDS = [
    ...
    def is_excluded(cls, title: str, company: str) -> bool:
        ...
        for pattern in cls.EXCLUDE_KEYWORDS:
            if re.search(pattern, title_lower):
                # Se disser "Junior / Senior" ou "Junior Specialist", trata
                if "junior" in title_lower or "trainee" in title_lower or "intern" in title_lower:
                    if pattern in [r"\bsenior\b", r"\bsr\.?\b", r"\blead\b", r"\bprincipal\b", r"\bstaff\b"]:
                        return True
                else:
                    return True
        return False
    ...
    def pre_filter_job(cls, job: JobPost) -> Optional[Tuple[JobPost, str]]:
        """Pré-filtra o cargo e categoriza entre IA/ML ou Top-Tier Software Engineering."""
        ...
        # 2. OBRIGATÓRIO: O título tem de conter Junior/Trainee/Intern/Estágio/Graduate/Bolsa
